...
export default function Hero() {
  ...
  return (
    <>
      {/* Header/Nav */}
      <header className="w-full z-30 bg-white/70 dark:bg-zinc-900/50 backdrop-blur border-b border-gray-200 dark:border-zinc-800">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav
            ...
            } lg:flex space-x-6 text-sm font-medium`}
          >
            {menuItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white pt-2"
              >
                {item.name}
              </Link>
            ))}
